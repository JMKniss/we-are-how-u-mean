...
import pandas as pd
from data.espn_client import get_draft_df, get_boxscores_df, invalidate_cache
...
with tab1:
    ...
    teams = ["All Teams"] + sorted(draft_df["team_name"].unique())
    selected = st.selectbox("Filter by team", teams)
    ...
    n_teams = draft_df["team_name"].nunique()
    n_rounds = draft_df["round"].max()
...
with tab2:
    ...
    # Per team picks
    selected_team = st.selectbox("Team", sorted(draft_df["team_name"].unique()), key="team_draft")
    ...
    # Join actual points if box score data available
    if not box_df.empty:
...
with tab3:
    ...
    if box_df.empty:
        ...
        fig = px.scatter(value_df, x="overall_pick", y="season_points",
                         color="team_name", hover_name="player_name",
                         title="Points Scored vs Draft Position",
                         labels={"overall_pick": "Draft Pick (Overall)", "season_points": "Season Points"})
        # Trend line
        import numpy as np
        ...
        # Worst busts (high pick, low points)
        busts = value_df[value_df["overall_pick"] <= 30].nsmallest(10, "season_points")[
            ["overall_pick", "round", "player_name", "team_name", "season_points"]
        ].round(2)
