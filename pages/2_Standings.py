...
with tab1:
    ...
    display = df[["team_name", "wins", "losses", "ties", "points_for", "points_against", "avg_score", "win_pct"]].copy()
    display.columns = ["Team", "W", "L", "T", "PF", "PA", "Avg Score", "Win%"]
    ...
    fig = px.bar(df, x="team_name", y=["wins", "losses"], barmode="group",
                 labels={"value": "Games", "team_name": "Team", "variable": ""},
...
with tab2:
    ...
    display = df[["team_name", "median_wins", "median_losses"]].copy()
    display.columns = ["Team", "Median Wins", "Median Losses"]
    st.dataframe(display, use_container_width=True)

    fig = px.bar(df, x="team_name", y="median_wins",
                 title="Median Wins per Team",
                 labels={"team_name": "Team", "median_wins": "Wins vs Median"},
...
with tab3:
    ...
    display = df[["team_name", "wins", "median_wins", "total_wins", "total_losses", "total_win_pct", "points_for"]].copy()
    display.columns = ["Team", "H2H W", "Median W", "Total W", "Total L", "Win%", "PF"]
    display["Win%"] = display["Win%"].round(3)
    display["PF"] = display["PF"].round(1)
    st.dataframe(display, use_container_width=True)
...
with tab4:
    ...
    display = df[["team_name", "avg_opp_score", "total_opp_score", "games"]].copy()
    display.columns = ["Team", "Avg Opp Score", "Total Opp Score", "Games Played"]
    display["Avg Opp Score"] = display["Avg Opp Score"].round(2)
    display["Total Opp Score"] = display["Total Opp Score"].round(1)
    st.dataframe(display, use_container_width=True)

    fig = px.bar(df, x="team_name", y="avg_opp_score",
                 title="Average Opponent Score Faced",
                 labels={"team_name": "Team", "avg_opp_score": "Avg Opp Score"},
...
with tab5:
    ...
    display = df[["team_name", "actual_wins", "expected_wins", "luck_score", "points_for"]].copy()
    display.columns = ["Team", "Actual W", "Expected W", "Luck Score", "PF"]
    display["Expected W"] = display["Expected W"].round(1)
    display["Luck Score"] = display["Luck Score"].round(2)
    display["PF"] = display["PF"].round(1)
    st.dataframe(display, use_container_width=True)

    fig = px.bar(df.sort_values("luck_score"), x="luck_score", y="team_name",
                 orientation="h",
                 title="Luck Score by Team",
                 labels={"luck_score": "Luck Score", "team_name": "Team"},
...
with tab6:
    ...
    display = df[["team_name", "alt_wins", "alt_games", "alt_win_pct"]].copy()
    display.columns = ["Team", "Alt Wins", "Games", "Win%"]
    display["Win%"] = display["Win%"].round(3)
    st.dataframe(display, use_container_width=True)

    # Compare actual vs alternate wins
    h2h = h2h_standings(matchups_df).set_index("team_name")["wins"]
    df["actual_wins"] = df["team_name"].map(h2h)
    fig = go.Figure()
    fig.add_trace(go.Bar(name="Actual Wins", x=df["team_name"], y=df["actual_wins"], marker_color="#3498db"))
    n_teams = matchups_df["team_id"].nunique()
    weeks_played = matchups_df["week"].nunique()
    scale = weeks_played / (n_teams - 1) if n_teams > 1 else 1
    fig.add_trace(go.Bar(name="Alt Wins (scaled)", x=df["team_name"],
